...
export default function OrdersPage() {
  ...
  const pastOrders = orders.filter(o => ['taken-unpaid', 'completed'].includes(o.status));
  ...
  const getStatusBadge = (status: string) => {
    ...
    return { style: styles[status as keyof typeof styles] || styles.received, label: labels[status as keyof typeof labels] || status };
  };
      ...
      <div className="container mx-auto px-6 py-6 max-w-2xl space-y-6">
        {orders.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h2 className="text-lg font-semibold text-neutral-900 mb-2">No orders yet</h2>
            <p className="text-neutral-500 mb-6">Start ordering your favorite meals</p>
          ...
          <>
            ...
            {activeOrders.length > 0 && (
              <div>
                <h2 className="text-sm font-medium text-neutral-500 mb-3">ACTIVE ORDERS</h2>
                <div className="space-y-3">
                  {activeOrders.map((order) => {
                    const badge = getStatusBadge(order.status);
                    return (
                      <div key={order.$id} className="bg-white border-2 border-[#86a349] rounded-2xl p-6">
                        <div className="flex items-start justify-between mb-4">
                          <div>
                            <div className="text-3xl font-bold text-neutral-900 mb-2">#{order.orderNumber}</div>
                            <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold border ${badge.style}`}>
                              {badge.label}
                            </span>
                            <div className="text-xs text-neutral-400 mt-2">{formatDate(order.createdAt)}</div>
                          </div>
                        </div>
                        ...
                        <div className="space-y-2 mb-4 py-4 border-y border-neutral-100">
                          {order.items.map((item, idx) => (
                            <div key={idx} className="flex justify-between text-sm">
                              ...
                              <span className="text-neutral-900">HK$ {item.price * item.quantity}</span>
                            </div>
                          ))}
                        </div>
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-xl font-semibold text-neutral-900">HK$ {order.total}</span>
                        </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {pastOrders.map((order) => {
                    const badge = getStatusBadge(order.status);
                    return (
                      <div key={order.$id} className="bg-white border border-neutral-100 rounded-xl p-4">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            <div className="font-semibold text-neutral-900 mb-1">HK$ {order.total}</div>
                            <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold border ${badge.style}`}>
                              {badge.label}
                            </span>
                          </div>
